...
2. The API is accessible at http://localhost:3003
...
1. If commands fail with connection errors:
   ...
   - Check if the API is accessible at http://localhost:3003
...
1. If the CLI can't connect to the API:
   ...
   - Verify the API URL is correct (default: http://localhost:3003)
   - You can override the API URL using the environment variable:
     ```bash
     docker run --network="host" -e API_URL=http://your-api-url:3003/api vigcoin-cli [command]
     ```
