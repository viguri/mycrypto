...
echo "Creating direct Plesk API fix for MyCrypto application..."
...
# 1. Create a robust vhost.conf for Plesk
echo "Creating robust vhost.conf for Plesk..."
mkdir -p /var/www/vhosts/viguri.org/crypto.viguri.org/conf
cat > /var/www/vhosts/viguri.org/crypto.viguri.org/conf/vhost.conf << 'VHOST'
...
# 2. Create a test PHP file to check API connectivity
...
echo "Direct Plesk API fix script created successfully!"
echo "To fix the API issues, run these commands:"
echo "1. Upload the script to your server:"
echo "   scp plesk_api_fix.sh viguri@81.169.168.33:/var/www/vhosts/viguri.org/crypto.viguri.org/"
echo "2. SSH to your server and make the script executable:"
echo "   ssh viguri@81.169.168.33"
echo "   cd /var/www/vhosts/viguri.org/crypto.viguri.org"
echo "   chmod +x plesk_api_fix.sh"
